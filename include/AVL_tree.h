...
private:
    ...
    Node* insert(Node* current, size_t value)
    {
        ...
        {
            throw logic_error("duplicate_value");
        }
        return balance(current);
    }

    Node* erase(Node* current, size_t value)
    {
        if (!current)
        {
            throw logic_error("value_not_found");
        }
        if (current->value > value)
        {
    ...
    Node* find(Node* current, size_t value)
    {
        if (!current)
        {
            return nullptr;
